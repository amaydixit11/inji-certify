...
import io.mosip.certify.core.dto.CredentialConfigurationMapping;
...
public class AuthorizationServerService {
    ...
    @Value("${mosip.certify.authorization.discovery.enabled:true}")
    private boolean discoveryEnabled;

    @Value("${mosip.certify.authorization.discovery.retry-count:3}")
    private int retryCount;

    @Value("${mosip.certify.authorization.discovery.timeout-seconds:10}")
    ...
    @Value("${mosip.certify.authorization.internal.enabled:true}")
    private boolean internalAuthServerEnabled;
    ...
    private void loadConfiguredServers() {
        // Add internal auth server if enabled
        if (internalAuthServerEnabled && StringUtils.hasText(internalAuthServerUrl)) {
    ...
    public AuthorizationServerMetadata discoverMetadata(String serverUrl) {
        ...
        if (!discoveryEnabled) {
            log.warn("Authorization server discovery is disabled");
            throw new CertifyException(ErrorConstants.AUTHORIZATION_SERVER_DISCOVERY_FAILED,
                    "Authorization server discovery is disabled");
        }
    ...
    /**
     * Refresh all AS metadata (useful for testing/admin operations)
     */
    public void refreshAllMetadata() {
        log.info("Refreshing metadata for all authorization servers");
        for (AuthorizationServerConfig config : configuredServers) {
            try {
                clearMetadataCache(config.getServerUrl());
                discoverMetadata(config.getServerUrl());
                log.info("Refreshed metadata for: {}", config.getServerUrl());
            } catch (Exception e) {
                log.error("Failed to refresh metadata for {}: {}",
    ...
    /**
     * Clear cached metadata for a specific server (useful for testing/refresh)
     */
    public void clearMetadataCache(String serverUrl) {
        vciCacheService.evictASMetadata(serverUrl);
        log.info("Cleared cached metadata for: {}", serverUrl);
    }

    /**
     * Clear all cached AS metadata
     */
    public void clearAllMetadataCache() {
        vciCacheService.clearASMetadataCache();
        log.info("Cleared all cached AS metadata");
    }
